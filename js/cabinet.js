...
function loadCabinet() {
  ...
  if (soundToggle && typeof soundEnabled !== 'undefined') soundToggle.checked = soundEnabled;
...
function loadBadges(stats) {
  ...
  badges.forEach(b => {
    if (b.cond() && !earnedBadges.includes(b.id)) {
      ...
      playSound("https://assets.mixkit.co/sfx/preview/mixkit-winning-chime-2015.mp3");
    }
  });
...
function editNick() {
  ...
  if (newNick.length < 3) return alert('Нікнейм повинен мати мінімум 3 символи');
  if (newNick.length > 20) return alert('Нікнейм не може перевищувати 20 символів');
...
function changePassword() {
  ...
  if (!newPass) return;
  if (newPass.length < 3) return alert('Пароль повинен мати мінімум 3 символи');
  user.pass = newPass;
...
async function addFriend() {
  ...
  try {
    ...
    alert('Помилка при додаванні друга');
  } finally {
    hideLoading();
  }
}
...
async function loadFriends() {
  ...
  try {
    ...
    friendsDiv.innerHTML = 'Помилка завантаження';
  }
}

function removeFriend(friendName) {
  if (confirm(`Ви дійсно хочете видалити ${friendName} з друзів?`)) {
...
// Експорт для main.js
window.saveThemeResult = saveThemeResult;
